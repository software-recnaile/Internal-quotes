...
def calculate_dimensions(mesh: trimesh.Trimesh):
    """Calculate dimensions of mesh with error handling"""
    try:
        # Ensure mesh has vertices
        if mesh.vertices is None or len(mesh.vertices) == 0:
            return {"x": 0.0, "y": 0.0, "z": 0.0}
        
        # Get bounds - handle None case
        bounds = mesh.bounds
        if bounds is None:
        ...
        try:
            ...
            if not isinstance(mesh, trimesh.Trimesh):
                try:
                    ...
                    if hasattr(mesh, 'dump'):
                        ...
                        # Handle trimesh.Scene
                        mesh = trimesh.util.concatenate(
                            list(trimesh.Trimesh(vertices=g.vertices, faces=g.faces) 
                                 for g in mesh.geometry.values())
                        )
                except Exception as e:
                    print(f"Error converting to mesh: {e}")
                    # Try to get first available mesh
            ...
            if not isinstance(mesh, trimesh.Trimesh):
                results.append({
                    ...
                    "dimensions_before": {"x": 0.0, "y": 0.0, "z": 0.0},
                    "dimensions_after": {"x": 0.0, "y": 0.0, "z": 0.0},
                })
                continue
            ...
            face_normals = mesh.face_normals if hasattr(mesh, 'face_normals') else []
            flat_faces = []
            ...
            if flat_faces and hasattr(mesh, 'area_faces'):
